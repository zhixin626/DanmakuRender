...
def _send_bark(bark_key, title, content, face_url=None, platform=None):
...
def bark_notify_url(room_url: str):
    """非阻塞调用，内部自动 3 次 retry"""
    def _notify_worker(room_url: str, max_retries: int = 3):
        for attempt in range(1, max_retries + 1):
            try:
                ...
                print(f"✅ Bark 推送成功：{streamer_info['name']} 开播")
                return
            except Exception as e:
                print(f"⚠️  Bark 推送失败（第 {attempt}/{max_retries} 次）：{e}")
                if attempt < max_retries:
                    time.sleep(2 ** attempt)  # 指数退避：2s, 4s
        print("❌ Bark 推送达到最大重试次数，放弃。")
    t = threading.Thread(
...
def bark_notify(title, content, face_url=None, platform=None):
    t = threading.Thread(
        target=_send_bark,
        args=(_BARK_KEY, title, content),
        kwargs={"face_url": face_url, "platform": platform},
        daemon=True # 主程序退出时，不等这个线程跑完，直接一起关掉
    )
    t.start()